...
saveBtn.onclick = async () => {
  ...
  // Validar campos de Tamanho e Preço
  sizesContainer.querySelectorAll('.input-group').forEach(div => {
    const [k, p] = div.querySelectorAll('input');
    if (!k.value || !p.value || isNaN(p.value)) {
      alert("Por favor, preencha todos os tamanhos e preços corretamente.");
      return;
    }
  ...
  if (inputFile.files.length) {
    ...
    if (!validTypes.includes(file.type)) {
      alert("Por favor, envie um arquivo de imagem válido (.jpg, .png).");
      return;
    }
